...
- Vitest testing infrastructure is established (`P2-T029`, `DEC-025`): Vitest 3 with `environment: node`, `globals: false` (explicit imports), `@vitest/coverage-v8` for V8 coverage, `supertest` for HTTP integration tests. Tests live under `tests/unit/` and `tests/integration/`. Logger is globally mocked in `tests/setup.ts`. `pnpm test` runs `vitest run`; `pnpm test:watch` runs interactive mode; `pnpm test:coverage` generates V8 coverage report. Jest is permanently dropped.

...
// Execute atomic status transition
...
const updateStatus = async (input: UpdateUserStatusInput): Promise<User> => {
  if (input.tx) {
    return await executeStatusUpdate(input.tx, input);
  }

  return await prisma.$transaction((tx) => executeStatusUpdate(tx, input));
};

// Execute atomic soft-delete lifecycle
...
const softDelete = async (input: SoftDeleteUserInput): Promise<User> => {
  if (input.tx) {
    return await executeSoftDelete(input.tx, input);
  }

  return await prisma.$transaction((tx) => executeSoftDelete(tx, input));
};
